...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
function slide({container,slide,nexArrow,prevArrow,totalCounter,currentCounter,wrapper,field}) {
    ...
    next.addEventListener('click', () => {
        if (offset == deteleNotDigits(width) * (slides.length - 1)) {
            offset = 0;
        } else {
        ...
        dots.forEach(dat => dat.style.opacity = '.5');
        dots[slideindex - 1].style.opacity = 1;
    });

    prev.addEventListener('click', () => {
        if (offset == 0) {
            offset = deteleNotDigits(width) * (slides.length - 1);
